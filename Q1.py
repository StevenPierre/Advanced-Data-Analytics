...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        e = (source, target)
        e_opp = (target, source)
        if e not in self.edges and e_opp not in self.edges:
            self.edges.append(e)
        # return NotImplemented
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for e in self.edges:
            print(e)
            if e[0] not in top_degree:
                top_degree[e[0]] = 1 
            else:
                top_degree[e[0]] += 1
            if e[1] not in top_degree:
                top_degree[e[1]] = 1
            else:
                top_degree[e[1]] +=1
        max_degree = 0
        for val in top_degree.values():
            if val > max_degree:
                max_degree = val 
        max_degree_dictionary = {}
        for n in top_degree:
            if top_degree[n] == max_degree:
                max_degree_dictionary[n] = max_degree
        return max_degree_dictionary
    
    def l_nodes(self) -> dict:
        degree = {}
        for e in self.edges:
            if e[0] not in degree:
                degree[e[0]] = 1
            else: 
                degree[e[0]] += 1
            if e[1] not in degree:
                degree[e[1]] = 1
            else:
                degree[e[1]] += 1
        return degree
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        data = response.read()
            ...
            #cast_data = json.loads(data).get('cast', [])
            
            #if exclude_ids: 
                #cast_data = [cast for cast in cast_data if cast['id'] not in exclude_ids]
        data_2 = json.loads(data.decode('utf-8'))
        cast_data = data_2['cast']
        if exclude_ids is not None:
            cast_data = [c for c in cast_data if c['id'] not in exclude_ids]
        if limit is not None:
            cast_data = [c for c in cast_data if c['order'] < limit]
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        credits_data = response2.read()

        #if response.status == 200:
            #movie_credits_data = json.loads(data).get('cast', [])
            #if vote_avg_threshold is not None:
                #movie_credits_data = [
                    #credit for credit in movie_credits_data if credit.get('vote_average', 0) >= vote_avg_threshold
                #]
            #movie_credits = [
                #{
                #    'id': credit['id'],
                #    'title': credit['title'],
                #    'vote_avg': credit.get('vote_average', 0)
                #}
               # for credit in movie_credits_data
            #]
        credits_data_to_ls = json.loads(credits_data.decode('utf-8'))
        movie_credits = credits_data_to_ls['cast']
        if vote_avg_threshold is not None:
            movie_credits = [c for c in movie_credits if c['vote_average'] >= vote_avg_threshold]        
...
tmdb = TMDBAPIUtils(api_key = '90fae1ff5fa4e542b52355386ba5e90d')
base_credits = tmdb.get_movie_credits_for_person(person_id='2975',vote_avg_threshold=8.0)
new_nodes_added = {}

for credit in base_credits:
    cast = tmdb.get_movie_cast(movie_id=str(credit['id']), limit=3)
    for c in cast:
        graph.add_node(id=str(c['id']),name=c['name'].replace(',',''))
        new_nodes_added[str(c['id'])] = c['name'].replace(',','')
        graph.add_edge(source ='2975', target =str(c['id']))
new_nodes_added.pop('2975')

new_nodes_added_2 = {}
for i in[1,2]:
    if i == 1:
        nodes = new_nodes_added
    else:
        nodes = new_nodes_added_2
    for node in nodes:
        credits = tmdb.get_movie_credits_for_person(person_id = node, vote_avg_threshold = 8.0)
        for cr in credits:
            cast = tmdb.get_movie_cast(movie_id=str(cr['id']), limit=3)
            for c in cast:
                if c['id'] not in graph.nodes:
                    graph.add_node(id=str(c['id']),name=c['name'].replace(',',''))
                    if i == 1:
                        new_nodes_added_2[str(c['id'])] = c['name'].replace(',','')
                if(node,str(c['id']))or(str(c['id']),node) not in graph.edges:
                    if node != str(c['id']):
                        graph.add_edge(source = node, target = str(c['id']))
#
