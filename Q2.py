...
class HW2_sql():
    ...
    def part_aii_1(self,connection,path):
        ...
        #cursor = connection.cursor()
        #path = "movies.csv"
        #data = open(path, enconding = 'utf8')
        with open(path, encoding = 'utf8') as f:
            reader = csv.reader(f)
            data = list(reader)
            #Insert_into = "INSERT INTO movies (id, title, score) VALUES (?, ?, ?)"
        #cursor.executemany(Insert_into, data_in_data)
        for row in data: 
            connection.execute("INSERT INTO movies (id, title, score) VALUES (?, ?, ?)" , (row[0], row[1], row[2]))
                
            #connection.commit()

        #select_star = "SELECT * FROM movies"
        #rows = cursor.execute(select_star).fetchall()
        #connection.commit()
       ######################################################################
    ...
    def part_aii_2(self,connection, path):
        ...
        with open(path, encoding = 'utf8') as f:
            reader = csv.reader(f)
            data = list(reader)
        #data_in_data = csv.reader(data)
            #Insert_into = "INSERT INTO movie_cast (movie_id, cast_id, cast_name, birthday, popularity) VALUES (?, ?, ?, ?, ?)"
        #cursor.executemany(Insert_into, data_in_data)
        for row in data: 
            connection.execute("INSERT INTO movie_cast (movie_id, cast_id, cast_name, birthday, popularity) VALUES (?, ?, ?, ?, ?)" , (row[0], row[1], row[2], row[3], row[4]))
                
            #connection.commit()
        #select_star = "SELECT * FROM movie_cast"
        #rows = cursor.execute(select_star).fetchall()
        #connection.commit()
        ######################################################################
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = "SELECT printf('%.2f', count(title)/(SELECT count(*) * 1.0 FROM movies)*100) as proportion FROM movies WHERE score between 7 AND 20"
        ######################################################################
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = "SELECT cast_name, COUNT(movie_id) AS appearances FROM movie_cast WHERE popularity > 10 GROUP BY cast_name ORDER BY appearances DESC, cast_name ASC LIMIT 5"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = """
        SELECT title, printf('%.2f', score) AS movie_score, COUNT(cast_id) AS cast_count
        FROM movies
        INNER JOIN movie_cast ON movie_cast.movie_id=movies.id
        GROUP BY movies.title
        ORDER BY score DESC, cast_count ASC, title ASC LIMIT 5
        """
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = """
        SELECT cast_id, cast_name, printf('%.2f',avg(score))
        FROM movies
        INNER JOIN movie_cast ON movie_cast.movie_id=movies.id
        WHERE movies.score >= 25
        GROUP BY movie_cast.cast_id
        HAVING COUNT(movie_id) >= 3
        ORDER BY AVG(score) DESC, cast_name ASC LIMIT 10
        """
        ######################################################################
    ...
    def part_gi(self,connection):
        ...
        part_g_i_sql = """
        SELECT 
        a.cast_id as cast_id,cast_bio.cast_name as cast_name,printf('%0.2f', avg(CAST(a.collaboration_score as FLOAT))) as collaboration_score
        FROM
        (SELECT
        cast_member_id1 as cast_id, average_movie_score as collaboration_score
        FROM
        good_collaboration
        UNION ALL
        SELECT
        cast_member_id2 as cast_id, average_movie_score as collaboration_score
        FROM
        good_collaboration
        ) as a
        INNER JOIN cast_bio
        ON cast_bio.cast_id = a.cast_id
        GROUP BY
        a.cast_id
        ORDER BY
